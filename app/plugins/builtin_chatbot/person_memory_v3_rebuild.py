"""Historical rebuild and atomic activation for person-memory v3.
...
def _clear_candidate_v3(store: MemoryStore, chat_name: str) -> None:
...
def _reject_bot_prompt_observations(
    ...
    rejected = 0
    for observation in v3.list_observations(
        chat_name,
...
def _build_batches(
    ...
    *,
    ...
    while index < total:
        ...
        used = 0
        while index + len(core) < total and len(core) < target_messages:
            message = messages[index + len(core)]
            cost = context_manager.estimate_message_tokens(message)
            if core and used + cost > input_token_budget:
                break
            core.append(message)
            used += cost
        if not core:
            core = [messages[index]]
        core_start_index = index
        core_end_index = index + len(core) - 1
        context_start = max(0, core_start_index - overlap)
        context_end = min(total, core_end_index + overlap + 1)
        selected = [
            dict(message) for message in messages[context_start:context_end]
        ]
        batches.append(
            {
                "namespace": namespace,
                "batch_index": len(batches) + 1,
                "core_start_cursor": int(core[0]["_log_cursor"]),
                "core_end_cursor": int(core[-1]["_log_cursor"]),
                "messages": selected,
            }
        )
        index += len(core)
...
def _configure_deepseek_manager(
    ...
    defaults = {
        "memory_person_observe": (8000, 180),
        "memory_person_period": (5000, 180),
        "memory_person_consolidate": (7000, 240),
    }
    for call_type, (max_tokens, timeout) in defaults.items():
        ...
        override.update(
            {
                "temperature": 0.0 if call_type == "memory_person_observe" else 0.1,
...
def _candidate_report(
    ...
    *,
    ...
    stats = v3.observation_stats(chat_name)
    profiles = v3.list_profiles(chat_name)
    with store._connection() as connection:
    ...
    return {
        ...
        "chat_name": chat_name,
        "schema_version": PERSON_MEMORY_V3_SCHEMA_VERSION,
...
def rebuild_candidate(
    *,
    ...
    v3 = PersonMemoryV3Store(store)
    if fresh:
        _clear_candidate_v3(store, chat_name)
    elif rebuild_derived:
        _clear_candidate_derived(store, chat_name)
    v3.ensure_chat_state(
        chat_name,
        mode="building",
    ...
    if person_centric:
        ...
        batches = [
            ...
            for batch in _build_batches(
                ...
                target_messages=max(20, min(240, int(target_messages))),
                overlap=max(0, min(60, int(overlap))),
    ...
    def budgeted_call_json(**kwargs: Any) -> Dict[str, Any]:
        ...
        estimated_output = (
            ...
            if call_type == "memory_person_observe"
            else 5000
        )
    ...
    def extract_with_split(
        ...
        *,
        ...
        result = engine.extract_observations(
            ...
            source_namespace=namespace,
            batch_key=(
                f"{namespace}:{core_start_cursor}:{core_end_cursor}:"
                f"d{depth}"
            ),
            excluded_sender_names=excluded_sender_names,
            excluded_sender_ids=excluded_sender_ids,
        ...
        if len(core) <= 24 or depth >= 3:
            raise RuntimeError(
                "observation extraction failed after split: "
                f"{namespace} {core_start_cursor}-{core_end_cursor}"
            )
        middle = len(core) // 2
        children = (core[:middle], core[middle:])
        results = []
        for child in children:
            child_start = int(child[0]["_log_cursor"])
            child_end = int(child[-1]["_log_cursor"])
            start_index = max(
                0,
                next(
                    index
                    for index, message in enumerate(ordered)
                    if int(message.get("_log_cursor") or 0) == child_start
                )
                - max(8, overlap),
            )
            end_index = min(
                ...
                next(
                    index
                    for index, message in enumerate(ordered)
                    if int(message.get("_log_cursor") or 0) == child_end
                )
                + max(8, overlap)
                + 1,
            )
        ...
        return {
            ...
            "quarantined": sum(item["quarantined"] for item in results),
            "filtered_low_value": sum(
                item["filtered_low_value"] for item in results
            ),
            "filtered_weak_evidence": sum(
                item["filtered_weak_evidence"] for item in results
            ),
            "filtered_verification": sum(
                item["filtered_verification"] for item in results
            ),
            "verification_quarantined": sum(
                item["verification_quarantined"] for item in results
            ),
            ...
            "split_depth": max(item["split_depth"] for item in results),
            "model_segments": sum(
                item["model_segments"] for item in results
            ),
        }

    def extract_batch(global_index: int, batch: Dict[str, Any]) -> Dict[str, Any]:
        namespace = str(batch["namespace"])
        destination = (
            batch_directory
            / f"{global_index:05d}-{namespace}.json"
        )
        if destination.is_file():
            return json.loads(destination.read_text(encoding="utf-8"))
        result = extract_with_split(
            namespace,
            batch["messages"],
            int(batch["core_start_cursor"]),
            int(batch["core_end_cursor"]),
        )
        record = {
            "global_batch_index": global_index,
            "namespace": namespace,
            "core_start_cursor": int(batch["core_start_cursor"]),
            "core_end_cursor": int(batch["core_end_cursor"]),
            "inserted": int(result.get("inserted") or 0),
            "quarantined": int(result.get("quarantined") or 0),
            "filtered_low_value": int(
                result.get("filtered_low_value") or 0
            ...
            "model_segments": int(result.get("model_segments") or 1),
            "completed_at": _now(),
        }
        with output_lock:
    ...
    try:
        if person_centric:
            ...
            def prepare_person_task(
                ...
                batch = v3.next_indexed_person_batch(
                    chat_name,
                    person_id,
                    namespace,
                ...
                def extract_segment(
                    ...
                    *,
                    ...
                    if not ordered_cursors:
                        raise RuntimeError(
                            "person-centric split produced no core cursors"
                        )
                    base_limit = max(
                        4,
                    ...
                    if len(ordered_cursors) <= 16 or depth >= 4:
                        raise RuntimeError(
                            "person-centric observation extraction failed "
                            "after split: "
            ...
            while len(exhausted) < len(state_by_key):
                ...
                with ThreadPoolExecutor(
                    ...
                    thread_name_prefix="person-v3-person",
                ) as executor:
                    futures = {
                ...
                for key, link_ids in wave_links.items():
                    v3.mark_indexed_person_batch_processed(
                        chat_name,
                        key[0],
                        key[1],
                        link_ids,
                ...
                print(
                    "person-v3 person-centric observations: "
                    f"batches={completed} "
            ...
            if completed:
                snapshot = budget.snapshot()
                print(
                    "person-v3 person-centric observations complete: "
            ...
            completed = sum(
                ...
                if (
                    batch_directory
                    / f"{index:05d}-{batch['namespace']}.json"
                ).is_file()
            )
            pending = [
                (index, batch)
            ...
            for wave_start in range(0, len(pending), wave_size):
                ...
                with ThreadPoolExecutor(
                    max_workers=worker_count,
                    thread_name_prefix="person-v3-observe",
                ) as executor:
                    futures = {
                        executor.submit(extract_batch, index, batch): index
        ...
        if bot_prompt_rejections:
            ...
            print(
                "person-v3 bot-prompt observations rejected: "
                f"{bot_prompt_rejections}",
                flush=True,
            )

        profiles_with_observations = v3.list_profiles(chat_name)
        ...
        for profile in profiles_with_observations:
            ...
            observations = v3.list_observations(
                chat_name,
                person_id=person_id,
                quality_status="active",
                limit=10000,
        ...
        def summarize_period(
            ...
            if destination.is_file():
                ...
                v3.upsert_period_summary(
                    ...
                    generator_version="person-v3.1-history",
                )
                return record
                ...
                *,
                ...
                try:
                    payload = budgeted_call_json(
                        call_type="memory_person_period",
        ...
        with ThreadPoolExecutor(
            ...
            thread_name_prefix="person-v3-period",
        ) as executor:
            futures = {
    ...
    report.update(
        {
            ...
            "person_centric": bool(person_centric),
...
def activate_candidate(
    *,
    ...
    backup = workspace / f"production-before-v3-activation-{timestamp}.db"
    ...
    production_v3 = PersonMemoryV3Store(production_store)
    PersonMemoryV3Store(candidate_store)
    ...
    try:
        ...
        id_maps = {
            ...
            for table in V3_AUTOINCREMENT_TABLES
        }
        ...
        try:
            ...
            for table in V3_TABLES:
                ...
                for source in candidate_rows[table]:
                    ...
                    elif table == "memory_person_v3_audit":
                        row["target_id"] = (
                            observation_map.get(
                    ...
                    elif table == "memory_person_v3_state":
                        row["mode"] = "active"
    ...
    state = production_v3.get_chat_state(chat_name)
    profiles = production_v3.list_profiles(chat_name)
    result = {
        "status": "activated",
        "chat_name": chat_name,
        "schema_version": PERSON_MEMORY_V3_SCHEMA_VERSION,
        ...
        "profile_count": len(profiles),
        "snapshot_count": sum(bool(item.get("snapshot_id")) for item in profiles),
        "observation_stats": production_v3.observation_stats(chat_name),
        "mode": state.get("mode"),
...
def catch_up_live_after_activation(
    *,
    ...
    """Index and consolidate messages appended after a V3 candidate was built.
    ...
    v3 = PersonMemoryV3Store(store)
    state = v3.get_chat_state(chat_name)
    if state.get("mode") != "active":
        raise RuntimeError(
            "person-memory v3 must be active before live catch-up"
        )
    with store._connection() as connection:
    ...
    backup = (
        workspace
        / f"production-before-v3-live-catchup-{timestamp}.db"
    )
    _backup_database(production_database, backup)
    result: Dict[str, Any] = {
        "status": "catching_up",
    ...
    try:
        ...
        if core_cursors:
            ...
            indexed = v3.index_person_messages(
                chat_name,
                selected,
        ...
        v3.set_chat_mode(chat_name, "active")
        result.update(
            {
                "status": "complete",
                ...
                "pending_observations_after":
                    pending_observations_after,
                "observation_stats": v3.observation_stats(chat_name),
                "profile_count": sum(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Rebuild person-memory v3 from raw historical messages",
    )
    ...
    parser.add_argument(
        "--workspace",
        default="data/person_rebuilds/person-v3",
    )
    ...
    parser.add_argument(
        "--legacy-group-batches",
        action="store_true",
        help="use the retired group-wide observation quota instead of person-centric queues",
    )
    parser.add_argument(
    ...
    if args.rollback_backup:
        ...
        result = rebuild_candidate(
            ...
            person_centric=not args.legacy_group_batches,
